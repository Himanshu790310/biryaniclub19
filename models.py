...
class MenuItem(db.Model):
    ...
    @property
    def has_image(self):
        """Check if menu item has an uploaded image"""
        return bool(self.image_filename)

    @property
    def image_url(self):
        """Get the full URL path for the menu item image"""
        if self.image_filename:
            return f"uploads/menu_items/{self.image_filename}"
        return None

    @property
    def veg_symbol(self):
        """Get vegetarian/non-vegetarian symbol"""
        return "🟢" if self.is_vegetarian else "🔴"

    @property
    def veg_label(self):
        """Get vegetarian/non-vegetarian label"""
        return "Veg" if self.is_vegetarian else "Non-Veg"
...
class Promotion(db.Model):
    ...
    discount_type = db.Column(db.String(20), nullable=False)  # 'percentage', 'fixed', or 'free_item_category'
    ...
    # Free item category fields
    free_item_category = db.Column(db.String(50), nullable=True)
    free_item_qty = db.Column(db.Integer, default=1)
    ...
    def is_expired(self):
        ...
        if self.expires_at:
            return ist_now() > self.expires_at
        return False
    ...
    def calculate_discount(self, subtotal, cart_items=None):
        """Calculate discount amount for given subtotal and cart items"""
        ...
        if self.discount_type == 'percentage':
            ...
            return discount, {'type': 'percentage', 'value': self.discount_value}
        elif self.discount_type == 'fixed':
            return min(self.discount_value, subtotal), {'type': 'fixed', 'value': self.discount_value}
        elif self.discount_type == 'free_item_category' and cart_items:
            # Find eligible items in the specified category
            eligible_items = []
            for item in cart_items:
                if item.menu_item.category == self.free_item_category:
                    for _ in range(item.quantity):
                        eligible_items.append(item.menu_item.price)

            if eligible_items:
                # Sort by price and take cheapest items up to free_item_qty
                eligible_items.sort()
                free_items_count = min(len(eligible_items), self.free_item_qty)
                discount = sum(eligible_items[:free_items_count])
                return discount, {
                    'type': 'free_item_category',
                    'category': self.free_item_category,
                    'qty': free_items_count,
                    'items_freed': free_items_count
                }
...
class CouponUsage(db.Model):
    """Track individual coupon usage events for admin visibility"""
    ...
    # User information
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=True)  # Nullable for guest orders
    username = db.Column(db.String(80), nullable=True)  # Store username at time of usage
    user_email = db.Column(db.String(120), nullable=True)  # Store email at time of usage
    ...
    coupon_code = db.Column(db.String(20), nullable=False)  # Store code at time of usage
    
    # Order information
    order_id = db.Column(db.Integer, db.ForeignKey('order.id'), nullable=False)
    order_number = db.Column(db.String(20), nullable=False)  # Store order number for easy reference
    
    # Financial details
    order_subtotal = db.Column(db.Float, nullable=False)  # Subtotal before discount
    discount_amount = db.Column(db.Float, nullable=False)  # Actual discount applied
    discount_type = db.Column(db.String(20), nullable=False)  # Type of discount applied
    
    # Usage details
    used_at = db.Column(db.DateTime, default=ist_now, nullable=False)  # When coupon was used
    customer_ip = db.Column(db.String(45), nullable=True)  # Store IP for fraud detection
    
    # Relationships
    user = db.relationship('User', backref='coupon_usage_history', lazy=True)
    promotion = db.relationship('Promotion', backref='usage_history', lazy=True)
    order = db.relationship('Order', backref='coupon_usage', lazy=True)
    
    @property
    def used_at_ist(self):
        """Return used_at time (already in IST)"""
        return self.used_at
    
    @property
    def is_guest_usage(self):
        """Check if this was a guest order usage"""
        return self.user_id is None
    
    @property
    def customer_display_name(self):
        """Get display name for the customer"""
        if self.is_guest_usage:
    ...
    def customer_contact_info(self):
        ...
        return {
            ...
            'name': self.user.full_name if self.user else self.username
        }
    
    @property
    def discount_percentage(self):
        """Calculate discount as percentage of order subtotal"""
        if self.order_subtotal > 0:
            return round((self.discount_amount / self.order_subtotal) * 100, 2)
        return 0.0
    
    def __repr__(self):
        return f'<CouponUsage {self.coupon_code} by {self.customer_display_name}>'
